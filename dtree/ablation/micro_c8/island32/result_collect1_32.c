...
__visible __xread uint32_t result_xfer2[7];

__visible SIGNAL result_signal3;
__visible __xread uint32_t result_xfer3[7];

__visible SIGNAL result_signal4;
__visible __xread uint32_t result_xfer4[7];

__visible SIGNAL result_signal5;
__visible __xread uint32_t result_xfer5[7];

__remote SIGNAL result_signal8;
__remote __xread uint32_t result_xfer8[7];

SIGNAL local_signal;

int main(void) {
    __gpr uint32_t results_gpr2[7];
    __gpr uint32_t results_gpr3[7];
    __gpr uint32_t results_gpr4[7];
    __gpr uint32_t results_gpr5[7];
    ...
    for (;;) {
        ...
        remote_me_reg_write_signal_remote(result_write, 32, 8, 0, 
                                          __xfer_reg_number(result_xfer8, __nfp_meid(32, 8)) + (__ctx() * 32), 
