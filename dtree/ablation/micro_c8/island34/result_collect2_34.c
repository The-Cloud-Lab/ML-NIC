...
__visible __xread uint32_t result_xfer6[7];

__visible SIGNAL result_signal7;
__visible __xread uint32_t result_xfer7[7];

__visible SIGNAL result_signal8;
__visible __xread uint32_t result_xfer8[7];
...
int main(void) {
    __gpr uint32_t results_gpr6[7];
    __gpr uint32_t results_gpr7[7];
    __gpr uint32_t results_gpr8[7];
    __xwrite uint32_t final_result_write;
    
    __lmem uint8_t path_class[] = {3, 0, 2, 2, 2, 1, 6, 6, 0, 0, 0, 6, 0, 0, 2, 0, 6, 6, 6, 0, 0, 6, 0, 2, 3, 2, 0, 2, 0, 0, 0, 0, 6, 0, 0, 0, 3, 5, 5, 2, 6, 3, 0, 0, 6, 6, 0, 0, 0, 0, 5, 0, 6, 0, 6, 4, 0, 0, 6, 0, 0, 6, 0, 6, 6, 0, 6, 6, 6, 0, 0, 3, 0, 0, 0, 0, 0, 3, 3, 3, 0, 0, 6, 0, 4, 0, 6, 6, 0, 0, 0, 6, 0, 0, 0, 0, 6, 0, 4, 3, 4, 4, 4, 0, 0, 3, 3, 0, 6, 0, 0, 2, 0, 6, 0, 3, 3, 0, 0, 6, 0, 0, 0, 3, 0, 3, 3, 3, 6, 0, 0, 0, 4, 0, 1, 0, 1, 0, 0, 0, 6, 0, 6, 6, 6, 0, 0, 3, 3, 0, 6, 0, 4, 4, 4, 0, 3, 3, 0, 0, 6, 0, 0, 6, 0, 4, 0, 0, 0, 0, 0, 0, 4, 0, 0, 0, 0, 6, 6, 0, 0, 0, 6, 0, 6, 0, 0, 4, 4, 0, 0, 0, 0, 6, 6, 0, 6, 6, 0, 0, 0, 6, };
    ...
    for (;;) {
        ...
        if (results_gpr8[0] != 0)
            results_gpr8[0] = ffs(results_gpr8[0]) + 192;
        else if (results_gpr8[1] != 0)
            results_gpr8[0] = ffs(results_gpr8[1]) + 160;
        else if (results_gpr8[2] != 0)
            results_gpr8[0] = ffs(results_gpr8[2]) + 128;
        else if (results_gpr8[3] != 0)
            results_gpr8[0] = ffs(results_gpr8[3]) + 96;        
        else if (results_gpr8[4] != 0)
            results_gpr8[0] = ffs(results_gpr8[4]) + 64;        
        else if (results_gpr8[5] != 0)
            results_gpr8[0] = ffs(results_gpr8[5]) + 32;        
        else
            results_gpr8[0] = ffs(results_gpr8[6]);        
