...
__intrinsic void proc_pkt(__mem40 char *buf_addr, __gpr uint32_t buf_off) {
    ...
    if (((__gpr uint16_t *)&eh.eth.dst)[0] == 0x0015
        ...
        if (eh.ip4.proto == 0x6) {  // dealing with TCP
            ...
            // Inform ME 1 that data is ready (ipv4 flags)
            remote_me_reg_write_signal_remote(data_out+6, 35, 1, 0, 
                                              __xfer_reg_number(&remote_xfer3, __nfp_meid(35, 1)) + (__ctx() * 32), 
                                              sizeof(remote_xfer3), &local_signal);

            // Inform ME 2 that data is ready (tcp destination port)
            remote_me_reg_write_signal_remote(data_out+10, 35, 2, 0, 
                                              __xfer_reg_number(&remote_xfer4, __nfp_meid(35, 2)) + (__ctx() * 32), 
                                              sizeof(remote_xfer4), &local_signal);     
            
            // Inform ME 3 that data is ready (ipv4 total packet length)
            remote_me_reg_write_signal_remote(data_out+5, 35, 3, 0, 
                                              __xfer_reg_number(&remote_xfer6, __nfp_meid(35, 3)) + (__ctx() * 32), 
                                              sizeof(remote_xfer6), &local_signal); 
            
            // Inform ME 4 that data is ready (tcp flags)
            remote_me_reg_write_signal_remote(data_out+13, 35, 4, 0, 
                                              __xfer_reg_number(&remote_xfer7, __nfp_meid(35, 4)) + (__ctx() * 32), 
                                              sizeof(remote_xfer7), &local_signal); 
            
            // Wait for ME 5 to finish
            ...
            // Inform ME 1 that data is ready (ipv4 flags)
            remote_me_reg_write_signal_remote(data_out+5, 35, 1, 0, 
                                              __xfer_reg_number(&remote_xfer3, __nfp_meid(35, 1)) + (__ctx() * 32), 
                                              sizeof(remote_xfer3), &local_signal);

            // Inform ME 2 that data is ready (tcp destination port)
            remote_me_reg_write_signal_remote(data_out+11, 35, 2, 0, 
                                              __xfer_reg_number(&remote_xfer4, __nfp_meid(35, 2)) + (__ctx() * 32), 
                                              sizeof(remote_xfer4), &local_signal);     
            
            // Inform ME 3 that data is ready (ipv4 total packet length)
            remote_me_reg_write_signal_remote(data_out+4, 35, 3, 0, 
                                              __xfer_reg_number(&remote_xfer6, __nfp_meid(35, 3)) + (__ctx() * 32), 
                                              sizeof(remote_xfer6), &local_signal); 
