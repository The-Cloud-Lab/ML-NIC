...
__intrinsic void proc_pkt(__mem40 char *buf_addr, __gpr uint32_t buf_off) {
    ...
    if (((__gpr uint16_t *)&eh.eth.dst)[0] == 0x0015
        ...
        if (eh.ip4.proto == 0x6) {  // dealing with TCP
            ...
            remote_me_reg_write_signal_remote(data_out+6, 34, 7, 0, 
                                              __xfer_reg_number(&remote_xfer3, __nfp_meid(34, 7)) + (__ctx() * 32), 
                                              sizeof(remote_xfer3), &local_signal);

            // Inform ME 8 that data is ready (tcp destination port)
            remote_me_reg_write_signal_remote(data_out+10, 34, 8, 0, 
                                              __xfer_reg_number(&remote_xfer4, __nfp_meid(34, 8)) + (__ctx() * 32), 
                                              sizeof(remote_xfer4), &local_signal);     
            
            // Inform ME 9 that data is ready (ipv4 total packet length)
            remote_me_reg_write_signal_remote(data_out+5, 34, 9, 0, 
                                              __xfer_reg_number(&remote_xfer6, __nfp_meid(34, 9)) + (__ctx() * 32), 
                                              sizeof(remote_xfer6), &local_signal); 
            
            // Inform ME 10 that data is ready (tcp flags)
            remote_me_reg_write_signal_remote(data_out+13, 34, 10, 0, 
                                              __xfer_reg_number(&remote_xfer7, __nfp_meid(34, 10)) + (__ctx() * 32), 
            ...
            // Inform ME 7 that data is ready (ipv4 flags)
            remote_me_reg_write_signal_remote(data_out+5, 34, 7, 0, 
                                              __xfer_reg_number(&remote_xfer3, __nfp_meid(34, 7)) + (__ctx() * 32), 
                                              sizeof(remote_xfer3), &local_signal);

            // Inform ME 8 that data is ready (tcp destination port)
            remote_me_reg_write_signal_remote(data_out+11, 34, 8, 0, 
                                              __xfer_reg_number(&remote_xfer4, __nfp_meid(34, 8)) + (__ctx() * 32), 
                                              sizeof(remote_xfer4), &local_signal);     
            
            // Inform ME 9 that data is ready (ipv4 total packet length)
            remote_me_reg_write_signal_remote(data_out+4, 34, 9, 0, 
                                              __xfer_reg_number(&remote_xfer6, __nfp_meid(34, 9)) + (__ctx() * 32), 
                                              sizeof(remote_xfer6), &local_signal); 
            
            // Inform ME 10 that data is ready (tcp flags)
            remote_me_reg_write_signal_remote(data_out+11, 34, 10, 0, 
                                              __xfer_reg_number(&remote_xfer7, __nfp_meid(34, 10)) + (__ctx() * 32), 
                                              sizeof(remote_xfer7), &local_signal); 
