...
__visible __xread uint32_t result_xfer6[6];

__visible SIGNAL result_signal7;
__visible __xread uint32_t result_xfer7[6];

__visible SIGNAL result_signal8;
__visible __xread uint32_t result_xfer8[6];
...
int main(void) {
    __gpr uint32_t results_gpr6[6];
    __gpr uint32_t results_gpr7[6];
    __gpr uint32_t results_gpr8[6];
    __xwrite uint32_t final_result_write;
    
    __lmem uint8_t path_class[] = {3, 0, 0, 2, 2, 2, 1, 6, 6, 0, 0, 0, 6, 0, 6, 0, 0, 2, 0, 6, 6, 6, 6, 0, 0, 6, 0, 2, 3, 2, 0, 2, 0, 0, 0, 0, 6, 0, 0, 0, 3, 5, 5, 0, 2, 6, 3, 0, 0, 6, 6, 0, 0, 0, 0, 5, 4, 0, 6, 0, 6, 4, 0, 0, 6, 0, 0, 6, 0, 6, 6, 0, 6, 6, 6, 0, 0, 3, 0, 0, 0, 0, 0, 0, 0, 3, 3, 3, 0, 0, 6, 0, 0, 4, 0, 6, 6, 0, 0, 0, 0, 6, 0, 0, 0, 0, 0, 6, 0, 6, 4, 3, 4, 4, 4, 0, 0, 3, 3, 0, 6, 0, 0, 2, 0, 0, 6, 0, 3, 3, 0, 0, 6, 0, 0, 0, 3, 0, 3, 3, 3, 6, 0, 0, 0, 0, 4, 0, 1, 0, 1, 0, 0, 0, 6, 0, 6, 6, 6, 0, 0, 0, 3, 3, 0, 6, 0, 4, 4, 4, 0, 3, 3, 0, 0, 6, };
    ...
    for (;;) {
        ...
        if (results_gpr8[0] != 0)
            results_gpr8[0] = ffs(results_gpr8[0]) + 160;
        else if (results_gpr8[1] != 0)
            results_gpr8[0] = ffs(results_gpr8[1]) + 128;
        else if (results_gpr8[2] != 0)
            results_gpr8[0] = ffs(results_gpr8[2]) + 96;
        else if (results_gpr8[3] != 0)
            results_gpr8[0] = ffs(results_gpr8[3]) + 64;        
        else if (results_gpr8[4] != 0)
            results_gpr8[0] = ffs(results_gpr8[4]) + 32;        
        else
            results_gpr8[0] = ffs(results_gpr8[5]);        
