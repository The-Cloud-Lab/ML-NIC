...
__remote __xread uint32_t remote_xfer1[2];

__remote SIGNAL remote_signal2;
__remote __xread uint32_t remote_xfer2[2];
...
__remote __xread uint32_t remote_xfer14[2];

__remote SIGNAL remote_signal15;
__remote __xread uint32_t remote_xfer15[2];

__remote SIGNAL remote_signal16;
__remote __xread uint32_t remote_xfer16[2];

__remote SIGNAL remote_signal17;
__remote __xread uint32_t remote_xfer17[2];

__remote SIGNAL remote_signal18;
__remote __xread uint32_t remote_xfer18;

__remote SIGNAL remote_signal19;
__remote __xread uint32_t remote_xfer19[3];
...
__remote __xread uint32_t remote_xfer26[2];
...
__remote __xread uint32_t remote_xfer30[2];

__remote SIGNAL remote_signal31;
__remote __xread uint32_t remote_xfer31[2];
...
__remote SIGNAL remote_signal37;
__remote __xread uint32_t remote_xfer37;

__remote SIGNAL remote_signal38;
__remote __xread uint32_t remote_xfer38[2];

__remote SIGNAL remote_signal39;
__remote __xread uint32_t remote_xfer39;

__remote SIGNAL remote_signal40;
__remote __xread uint32_t remote_xfer40;

__remote SIGNAL remote_signal41;
__remote __xread uint32_t remote_xfer41;

__remote SIGNAL remote_signal42;
__remote __xread uint32_t remote_xfer42;

__remote SIGNAL remote_signal43;
__remote __xread uint32_t remote_xfer43[2];

__remote SIGNAL remote_signal44;
__remote __xread uint32_t remote_xfer44[3];
...
__intrinsic void proc_pkt(__mem40 char *buf_addr, __gpr uint32_t buf_off) {
    ...
    __xread uint32_t pkt_buf[24];
    __xwrite uint32_t data_out[24];
    ...
    if (((__gpr uint16_t *)&eh.eth.dst)[0] == 0x0015
        ...
        mem_read64(pkt_buf, buf_addr + buf_off + 50, 96);
        reg_cp(data_out, pkt_buf, 64);  // copy over features 1 - 32
        reg_cp(data_out+16, pkt_buf+16, 32);  // copy over feature 33 - 48
        ...
        remote_me_reg_write_signal_remote(data_out+2, 34, 11, 0, 
                                          __xfer_reg_number(&remote_xfer35, __nfp_meid(34, 11)) + (__ctx() * 32), 
                                          sizeof(remote_xfer35), &local_signal); 
       
        // Inform 35 - 0 that data is ready
        remote_me_reg_write_signal_remote(data_out+3, 35, 0, 0, 
                                          __xfer_reg_number(&remote_xfer36, __nfp_meid(35, 0)) + (__ctx() * 32), 
                                          sizeof(remote_xfer36), &local_signal);  
        
        // Inform 35 - 1 that data is ready
        remote_me_reg_write_signal_remote(data_out+4, 35, 1, 0, 
                                          __xfer_reg_number(&remote_xfer37, __nfp_meid(35, 1)) + (__ctx() * 32), 
                                          sizeof(remote_xfer37), &local_signal); 
        
        // Inform 35 - 2 that data is ready
        remote_me_reg_write_signal_remote(data_out+5, 35, 2, 0, 
                                          __xfer_reg_number(remote_xfer38, __nfp_meid(35, 2)) + (__ctx() * 32), 
                                          sizeof(remote_xfer38), &local_signal);
        
        // Inform 35 - 3 that data is ready
        remote_me_reg_write_signal_remote(data_out+7, 35, 3, 0, 
                                          __xfer_reg_number(&remote_xfer39, __nfp_meid(35, 3)) + (__ctx() * 32), 
                                          sizeof(remote_xfer39), &local_signal);
        
        // Inform 35 - 4 that data is ready
        remote_me_reg_write_signal_remote(data_out+8, 35, 4, 0, 
                                          __xfer_reg_number(&remote_xfer40, __nfp_meid(35, 4)) + (__ctx() * 32), 
                                          sizeof(remote_xfer40), &local_signal);
        
        // Inform 35 - 5 that data is ready
        remote_me_reg_write_signal_remote(data_out+9, 35, 5, 0, 
                                          __xfer_reg_number(&remote_xfer41, __nfp_meid(35, 5)) + (__ctx() * 32), 
                                          sizeof(remote_xfer41), &local_signal);
        
        // Inform 35 - 6 that data is ready
        remote_me_reg_write_signal_remote(data_out+10, 35, 6, 0, 
                                          __xfer_reg_number(&remote_xfer42, __nfp_meid(35, 6)) + (__ctx() * 32), 
                                          sizeof(remote_xfer42), &local_signal);
        
        // Inform 35 - 7 that data is ready
        remote_me_reg_write_signal_remote(data_out+11, 35, 7, 0, 
                                          __xfer_reg_number(remote_xfer43, __nfp_meid(35, 7)) + (__ctx() * 32), 
                                          sizeof(remote_xfer43), &local_signal);
        
        // Inform 35 - 8 that data is ready
        remote_me_reg_write_signal_remote(data_out+13, 35, 8, 0, 
                                          __xfer_reg_number(remote_xfer44, __nfp_meid(35, 8)) + (__ctx() * 32), 
                                          sizeof(remote_xfer44), &local_signal);    


        
        // Wait for 36 - 11 to finish
