...
__remote SIGNAL result_signal48;
__remote __xread uint32_t result_xfer48[8];
...
int main(void) {
    ...
    for (;;) {
        ...
        remote_me_reg_write_signal_remote(interm_result_write, 36, 8, 0, __xfer_reg_number(result_xfer48, __nfp_meid(36, 8)) + (__ctx() * 32), sizeof(result_xfer48), &local_signal);    
    }
