...
__remote SIGNAL result_signal54;
__remote __xread uint32_t result_xfer54[8];
...
int main(void) {
    ...
    __assign_relative_register(&result_signal37, 2);
    __assign_relative_register(&result_signal38, 3);
    __assign_relative_register(&result_signal39, 4);
    __assign_relative_register(&result_signal40, 5);
    ...
    for (;;) {
        ...
        remote_me_reg_write_signal_remote(interm_result_write, 36, 10, 0, __xfer_reg_number(result_xfer54, __nfp_meid(36, 10)) + (__ctx() * 32), sizeof(result_xfer54), &local_signal);    
    }
