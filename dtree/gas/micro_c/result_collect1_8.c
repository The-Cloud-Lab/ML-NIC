...
__remote SIGNAL result_signal52;
__remote __xread uint32_t result_xfer52[8];
...
int main(void) {
    ...
    for (;;) {
        ...
        remote_me_reg_write_signal_remote(interm_result_write, 36, 9, 0, __xfer_reg_number(result_xfer52, __nfp_meid(36, 9)) + (__ctx() * 32), sizeof(result_xfer52), &local_signal);    
    }
