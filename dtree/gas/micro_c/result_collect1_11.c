...
__remote SIGNAL result_signal55;
__remote __xread uint32_t result_xfer55[8];
...
int main(void) {
    ...
    __assign_relative_register(&result_signal41, 2);
    __assign_relative_register(&result_signal42, 3);
    __assign_relative_register(&result_signal43, 4);
    __assign_relative_register(&result_signal44, 5);
    ...
    for (;;) {
        ...
        remote_me_reg_write_signal_remote(interm_result_write, 36, 10, 0, __xfer_reg_number(result_xfer55, __nfp_meid(36, 10)) + (__ctx() * 32), sizeof(result_xfer55), &local_signal);    
    }
