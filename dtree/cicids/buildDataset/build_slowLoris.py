...
random.seed(99)
slowloris_pcap = "/home/admin2/ML_NIC/datasets/cicids2017/slowLoris.pcap"
...
slowLorisTrain = trainFlows[trainFlows['label'] == "dos_slowloris"].reset_index().drop(columns=['Unnamed: 0'])
slowLorisTest = testFlows[testFlows['label'] == "dos_slowloris"].reset_index().drop(columns=['Unnamed: 0'])
...
test_pcap = "/home/admin2/ML_NIC/datasets/cicids2017/slowLorisTest.pcap"

test_writer = PcapWriter(test_pcap)
pcap_reader = PcapReader(slowloris_pcap)
train_collect = {i:[] for i in range(len(slowLorisTrain))}
test_index = 0
for pkt in pcap_reader:
    ...
    trainMatch = slowLorisTrain[(tt >= slowLorisTrain['flow start time']) & (tt <= slowLorisTrain['flow end time']) & (slowLorisTrain['src port'] == src_port) & (slowLorisTrain['dst port'] == dst_port) & (slowLorisTrain['IP proto'] == ipProto)]
    testMatch = slowLorisTest[(tt >= slowLorisTest['flow start time']) & (tt <= slowLorisTest['flow end time']) & (slowLorisTest['src port'] == src_port) & (slowLorisTest['dst port'] == dst_port) & (slowLorisTest['IP proto'] == ipProto)]
    ...
    if (len(trainMatch) == 1):
        ...
        test_df.at[test_index, "label"] = "dos_slowloris"
...
for pkts in train_collect.values():
    ...
    train_df.at[train_index, "label"] = "dos_slowloris"
    train_index += 1
...
train_df.to_csv("/home/admin2/ML_NIC/datasets/cicids2017/slowLorisTrain.csv", index=False)
test_df.to_csv("/home/admin2/ML_NIC/datasets/cicids2017/slowLorisTest.csv", index=False)
