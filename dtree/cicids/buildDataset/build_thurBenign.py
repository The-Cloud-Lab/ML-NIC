...
random.seed(99)
thurbenign_pcap = "/home/admin2/ML_NIC/datasets/cicids2017/thurBenign.pcap"
...
thurBenignTrain = trainFlows[(trainFlows['label'] == "benign") & (trainFlows['flow start time'].dt.day == 6)].reset_index().drop(columns=['Unnamed: 0'])
thurBenignTest = testFlows[(testFlows['label'] == "benign") & (testFlows['flow start time'].dt.day == 6)].reset_index().drop(columns=['Unnamed: 0'])
...
test_pcap = "/home/admin2/ML_NIC/datasets/cicids2017/thurBenignTest.pcap"

test_writer = PcapWriter(test_pcap)
pcap_reader = PcapReader(thurbenign_pcap)
train_collect = {i:[] for i in range(len(thurBenignTrain))}
test_index = 0
for i, pkt in enumerate(pcap_reader):
    ...
    trainMatch = thurBenignTrain[(tt >= thurBenignTrain['flow start time']) & (tt <= thurBenignTrain['flow end time']) & (thurBenignTrain["IP proto"] == ipProto) & (((thurBenignTrain["src IP"] == ipSrc) & (thurBenignTrain["dst IP"] == ipDst) & (thurBenignTrain["src port"] == src_port) & (thurBenignTrain["dst port"] == dst_port)) | ((thurBenignTrain["dst IP"] == ipSrc) & (thurBenignTrain["src IP"] == ipDst) & (thurBenignTrain["dst port"] == src_port) & (thurBenignTrain["src port"] == dst_port)))]
    testMatch = thurBenignTest[(tt >= thurBenignTest['flow start time']) & (tt <= thurBenignTest['flow end time']) & (thurBenignTest["IP proto"] == ipProto) & (((thurBenignTest["src IP"] == ipSrc) & (thurBenignTest["dst IP"] == ipDst) & (thurBenignTest["src port"] == src_port) & (thurBenignTest["dst port"] == dst_port)) | ((thurBenignTest["dst IP"] == ipSrc) & (thurBenignTest["src IP"] == ipDst) & (thurBenignTest["dst port"] == src_port) & (thurBenignTest["src port"] == dst_port)))]
...
train_df.to_csv("/home/admin2/ML_NIC/datasets/cicids2017/thurBenignTrain.csv", index=False)
test_df.to_csv("/home/admin2/ML_NIC/datasets/cicids2017/thurBenignTest.csv", index=False)
