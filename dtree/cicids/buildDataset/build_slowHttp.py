...
random.seed(99)
slowhttp_pcap = "/home/admin2/ML_NIC/datasets/cicids2017/slowHttp.pcap"
...
slowHttpTrain = trainFlows[trainFlows['label'] == "dos_slowhttptest"].reset_index().drop(columns=['Unnamed: 0'])
slowHttpTest = testFlows[testFlows['label'] == "dos_slowhttptest"].reset_index().drop(columns=['Unnamed: 0'])
...
test_pcap = "/home/admin2/ML_NIC/datasets/cicids2017/slowHttpTest.pcap"

test_writer = PcapWriter(test_pcap)
pcap_reader = PcapReader(slowhttp_pcap)
train_collect = {i:[] for i in range(len(slowHttpTrain))}
test_index = 0
for pkt in pcap_reader:
    ...
    trainMatch = slowHttpTrain[(tt >= slowHttpTrain['flow start time']) & (tt <= slowHttpTrain['flow end time']) & (slowHttpTrain['src port'] == src_port) & (slowHttpTrain['dst port'] == dst_port) & (slowHttpTrain['IP proto'] == ipProto)]
    testMatch = slowHttpTest[(tt >= slowHttpTest['flow start time']) & (tt <= slowHttpTest['flow end time']) & (slowHttpTest['src port'] == src_port) & (slowHttpTest['dst port'] == dst_port) & (slowHttpTest['IP proto'] == ipProto)]
    ...
    if (len(trainMatch) == 1):
        ...
        test_df.at[test_index, "label"] = "dos_slowhttptest"
...
for pkts in train_collect.values():
    ...
    train_df.at[train_index, "label"] = "dos_slowhttptest"
    train_index += 1
...
train_df.to_csv("/home/admin2/ML_NIC/datasets/cicids2017/slowHttpTrain.csv", index=False)
test_df.to_csv("/home/admin2/ML_NIC/datasets/cicids2017/slowHttpTest.csv", index=False)
