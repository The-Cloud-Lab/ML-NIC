...
__remote __xread uint32_t result_xfer7[3];

SIGNAL local_signal;

// __cls uint32_t rcv_ftr = 0;
...
int main(void) {
    ...
    for (;;) {
        ...
        remote_me_reg_write_signal_remote(&result_write, 35, 6, 0, 
                                          __xfer_reg_number(result_xfer7, __nfp_meid(35, 6)) + (__ctx() * 32), 
