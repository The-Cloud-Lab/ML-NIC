...
__visible __xread uint32_t result_xfer2[3];

__visible SIGNAL result_signal3;
__visible __xread uint32_t result_xfer3[3];

__visible SIGNAL result_signal4;
__visible __xread uint32_t result_xfer4[3];

__visible SIGNAL result_signal6;
__visible __xread uint32_t result_xfer6[3];

__visible SIGNAL result_signal7;
__visible __xread uint32_t result_xfer7[3];
...
int main(void) {
    __gpr uint32_t results_gpr2[3];
    __gpr uint32_t results_gpr3[3];
    __gpr uint32_t results_gpr4[3];
    __gpr uint32_t results_gpr6[3];
    __gpr uint32_t results_gpr7[3];
    __xwrite uint32_t final_result_write;
    
    __lmem uint8_t path_class[] = {0, 6, 0, 0, 6, 3, 0, 0, 0, 6, 0, 0, 6, 0, 0, 0, 0, 3, 2, 0, 0, 2, 1, 2, 0, 6, 0, 2, 0, 2, 0, 6, 0, 1, 0, 3, 0, 3, 2, 3, 2, 3, 2, 0, 3, 5, 4, 5, 4, 0, 0, 0, 0, 0, 2, 0, 5, 0, 0, 0, 0, 0, 0, 0, 6, 6, 0, 0, 6, 0, 6, 0, 6, 0, 6, 0, 6, 0, 0, 0, 6, 6, 6, 0, 6, 0, 0, 0, 6, };    
    ...
    __assign_relative_register(&result_signal6, 5);
    __assign_relative_register(&result_signal7, 6);    
    ...
    for (;;) {
        __wait_for_all(&result_signal2, &result_signal3, &result_signal4, &result_signal6, &result_signal7);
        ...
        // Compute final path
        results_gpr7[0] = results_gpr2[0] & results_gpr3[0] & results_gpr4[0] & results_gpr6[0] & results_gpr7[0];
        results_gpr7[1] = results_gpr2[1] & results_gpr3[1] & results_gpr4[1] & results_gpr6[1] & results_gpr7[1];
        results_gpr7[2] = results_gpr2[2] & results_gpr3[2] & results_gpr4[2] & results_gpr6[2] & results_gpr7[2];
        
        // Calculate leaf node index
        if (results_gpr7[0] != 0)
            results_gpr7[0] = ffs(results_gpr7[0]) + 64;
        else if (results_gpr7[1] != 0)
            results_gpr7[0] = ffs(results_gpr7[1]) + 32;
        else
            results_gpr7[0] = ffs(results_gpr7[2]);        
        
        // Get class prediction
        results_gpr7[0] = (uint32_t)(path_class[results_gpr7[0]]);
        reg_cp(&final_result_write, results_gpr7, sizeof(final_result_write));

        remote_me_reg_write_signal_remote(&final_result_write, 34, 0, 0, 
                                          __xfer_reg_number(&complete_xfer, __nfp_meid(34, 0)) + (__ctx() * 32), 
