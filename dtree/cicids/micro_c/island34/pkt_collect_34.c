...
__intrinsic void proc_pkt(__mem40 char *buf_addr, __gpr uint32_t buf_off) {
    ...
    if (((__gpr uint16_t *)&eh.eth.dst)[0] == 0x0015
        ...
        if (eh.ip4.proto == 0x6) {  // dealing with TCP
            ...
            remote_me_reg_write_signal_remote(data_out+5, 34, 4, 0, 
                                              __xfer_reg_number(&remote_xfer6, __nfp_meid(34, 4)) + (__ctx() * 32), 
            ...
            remote_me_reg_write_signal_remote(data_out+13, 34, 5, 0, 
                                              __xfer_reg_number(&remote_xfer7, __nfp_meid(34, 5)) + (__ctx() * 32), 
            ...
            // Inform ME 4 that data is ready (ipv4 total packet length)
            remote_me_reg_write_signal_remote(data_out+4, 34, 4, 0, 
                                              __xfer_reg_number(&remote_xfer6, __nfp_meid(34, 4)) + (__ctx() * 32), 
                                              sizeof(remote_xfer6), &local_signal); 
            
            // Inform ME 5 that data is ready (tcp flags)
            remote_me_reg_write_signal_remote(data_out+11, 34, 5, 0, 
                                              __xfer_reg_number(&remote_xfer7, __nfp_meid(34, 5)) + (__ctx() * 32), 
                                              sizeof(remote_xfer7), &local_signal); 
