...
__intrinsic void proc_pkt(__mem40 char *buf_addr, __gpr uint32_t buf_off) {
    ...
    if (((__gpr uint16_t *)&eh.eth.dst)[0] == 0x0015
        ...
        remote_me_reg_write_signal_remote(data_out+11, 33, 1, 0, 
                                          __xfer_reg_number(&remote_xfer1, __nfp_meid(33, 1)) + (__ctx() * 32), 
                                          sizeof(remote_xfer1), &local_signal); 
        
        // Inform ME 2 that data is ready
        remote_me_reg_write_signal_remote(data_out+11, 33, 2, 0, 
                                          __xfer_reg_number(&remote_xfer2, __nfp_meid(33, 2)) + (__ctx() * 32), 
                                          sizeof(remote_xfer2), &local_signal);
        
        // Inform ME 3 that data is ready
        remote_me_reg_write_signal_remote(data_out+12, 33, 3, 0, 
                                          __xfer_reg_number(&remote_xfer3, __nfp_meid(33, 3)) + (__ctx() * 32), 
                                          sizeof(remote_xfer3), &local_signal);
        
        // Inform ME 4 that data is ready
        remote_me_reg_write_signal_remote(data_out+12, 33, 4, 0, 
                                          __xfer_reg_number(&remote_xfer4, __nfp_meid(33, 4)) + (__ctx() * 32), 
                                          sizeof(remote_xfer4), &local_signal);
        
        // Inform ME 5 that data is ready
        remote_me_reg_write_signal_remote(data_out+13, 33, 5, 0, 
                                          __xfer_reg_number(&remote_xfer5, __nfp_meid(33, 5)) + (__ctx() * 32), 
                                          sizeof(remote_xfer5), &local_signal);
        
        // Inform ME 6 that data is ready
        remote_me_reg_write_signal_remote(data_out+13, 33, 6, 0, 
                                          __xfer_reg_number(&remote_xfer6, __nfp_meid(33, 6)) + (__ctx() * 32), 
                                          sizeof(remote_xfer6), &local_signal);
        
        // Inform ME 7 that data is ready
        remote_me_reg_write_signal_remote(data_out+14, 33, 7, 0, 
                                          __xfer_reg_number(&remote_xfer7, __nfp_meid(33, 7)) + (__ctx() * 32), 
                                          sizeof(remote_xfer7), &local_signal);        
        
        // Inform ME 8 that data is ready
        remote_me_reg_write_signal_remote(data_out+14, 33, 8, 0, 
                                          __xfer_reg_number(&remote_xfer8, __nfp_meid(33, 8)) + (__ctx() * 32), 
