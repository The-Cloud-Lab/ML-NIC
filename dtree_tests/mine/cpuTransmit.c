...
int main(int argc, char **argv) {
        ...
        char *pcap_file = "../dtree_tests/mine/cpuTest.pcap";
	...
	// Open file to store model predictions
        pred_file = fopen("../dtree_tests/mine/cpu_pred_test.txt", "w");
        if (pred_file == NULL) {
        ...
        latency_file = fopen("../dtree_tests/mine/cpu_latT50.txt", "w");
...
void eff_handler(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
        ...
        struct pcap_pkthdr *pred_pktHdr;
        const u_char *pred_pkt;
