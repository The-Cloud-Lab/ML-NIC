...
int main(int argc, char **argv) {
        ...
        char *pcap_file = "../dtree_tests/cicids/cpuTest.pcap";
        char error[PCAP_ERRBUF_SIZE];
        pcap_t *pcap_handle, *network_handle;

        // Variables for packet filtering
        struct bpf_program filter;
        char filter_exp[] = "ether src <src_mac> and ether dst <dst_mac> and (ether proto 0x4d49 or udp)";
        ...
        pred_file = fopen("../dtree_tests/cicids/cpu_pred_test.txt", "w");
        if (pred_file == NULL) {
        ...
        latency_file = fopen("../dtree_tests/cicids/cpu_latN99.txt", "w");
        ...
        if (pcap_loop(pcap_handle, 6957375, eff_handler, (u_char *)network_handle) != 0) {
...
void eff_handler(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
        ...
        if (recv_ip_hdr->ip_p == IPPROTO_TCP) {
                payload = pred_pkt + ether_hdr_len + ip_hdr_len + tcp_hdr_len;
                fprintf(pred_file, "%u\n", *payload);

        } else if (recv_ip_hdr->ip_p == IPPROTO_UDP) {
                payload = pred_pkt + ether_hdr_len + ip_hdr_len + udp_hdr_len;
                fprintf(pred_file, "%u\n", *payload);

        } else if (recv_ip_hdr->ip_p == IPPROTO_ICMP || recv_ip_hdr->ip_p == IPPROTO_IGMP) {
                payload = pred_pkt + ether_hdr_len + ip_hdr_len + icmp_igmp_hdr_len;
                fprintf(pred_file, "%u\n", *payload);
