...
int main(int argc, char **argv) {
        ...
        char *pcap_file = "../dtree_tests/gas/cpuTest.pcap";
	...
	// Open file to store model predictions
        pred_file = fopen("../dtree_tests/gas/cpu_pred.txt", "w");
        if (pred_file == NULL) {
        ...
        latency_file = fopen("../dtree_tests/gas/cpu_latN99.txt", "w");
...
void eff_handler(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
        ...
        while (pcap_sendpacket(network_handle, packet, header->caplen) != 0) {
