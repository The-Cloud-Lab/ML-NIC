...
On the other server, please construct the packets for the corresponding task using the build_<task>.py file and save them to a PCAP file. Please note that to build the dataset we used for the cicids task, you can find code for labeling the raw dataset in dtree/cicids/buildDataset/. After running those steps, please return to dtree_tests/build_cicids.py to build the packets for the task. The effectiveness and latency raw data for our Micro-C implementation can then be collected using the nicTransmit.c file. The compute_metrics.py file can be used to calculate the metrics used in our work. For more details on our experiments, please refer to our paper. 
